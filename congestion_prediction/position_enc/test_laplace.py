...
config.posenc_LapPE.model = 'DeepSet'
config.posenc_LapPE.post_layers = 2
...
data = compute_posenc_stats(data, ['LapPE'], True, config)

# Use Laplace position encoder
from laplace_pos_encoder import LapPENodeEncoder

module = LapPENodeEncoder(cfg = config, dim_in = 1, dim_emb = 20, expand_x = True)
