...
config.posenc_RWSE.kernel.times = [1, 2, 3, 4, 5]
config.posenc_RWSE.model = 'mlp'
config.posenc_RWSE.post_layers = 2
...
data = compute_posenc_stats(data, ['RWSE'], True, config)

# Use kernel position encoder
from kernel_pos_encoder import KernelPENodeEncoder

module = KernelPENodeEncoder(kernel_type = 'RWSE', cfg = config, dim_in = 1, dim_emb = 20, expand_x = True)
