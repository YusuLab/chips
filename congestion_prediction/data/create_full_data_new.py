...
for sample in range(N):
    ...
    for net in net_dict:
        if net not in drive_dict:
            ...
            index = drive_dict[net]
            drive_inst = instances[index]

            follow_indices = net_dict[net]   
            follow_insts = [instances[i] for i in follow_indices]
            follow_attrs = [attrs[i] for i in follow_indices]

            for idx in range(len(follow_insts)):
                follow_inst = follow_insts[idx]
                follow_attr = follow_attrs[idx]
                n_row.append(drive_inst)
                n_col.append(follow_inst)
                n_edge_attr.append(follow_attr)
    
    
    
    dictionary['edge_attr'] = np.array(n_edge_attr)
    dictionary['edge_index'] = np.array([n_row, n_col])
