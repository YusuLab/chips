...
# Dataset
print(args.data_dir)
dataset = pyg_dataset(data_dir = args.data_dir, graph_index = args.graph_index, target = args.target, load_pe = load_pe, num_eigen = num_eigen, load_global_info = load_global_info, load_pd = load_pd, graph_rep = "star")
...
for epoch in range(num_epoch):
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        loss = weighted_mse(predict.view(-1), targets.view(-1), weights)
        mse_loss = F.mse_loss(predict.view(-1), targets.view(-1))
