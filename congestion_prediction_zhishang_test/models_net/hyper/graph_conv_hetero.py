...
class HyperConv(MessagePassing):
    def __init__(self, out_channels, edge_dim):
        ...
        self.mlp = Seq(Linear(out_channels * 3, out_channels * 3),
                       ...
                       ReLU(),
                       Linear(out_channels * 3, out_channels * 2),
    ...
    def forward(self, x, edge_index, edge_attr, edge_index_v_drive, edge_index_v_sink):
        
        edge_embedding = self.edge_encoder(edge_attr)

        # here the edge_index should be [[inst_idx], [net_idx]], which represent edges from instances to nets
        net_agg = self.propagate(edge_index, edge_type="inst_to_net", x=x, edge_attr=None) 
        
        # after we get the net aggregation, we passed the net's aggregated information back to instances
        h_drive = self.propagate(edge_index_v_drive.flip(0), edge_type="net_to_inst_v_drive", x=net_agg, edge_attr=None)
        h_sink = self.propagate(edge_index_v_sink.flip(0), edge_type="net_to_inst_v_sink", x=net_agg, edge_attr=edge_embedding)
