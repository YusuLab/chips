...
print('min xloc:', min(xloc_list))
print('max xloc:', max(xloc_list))
print('min yloc:', min(yloc_list))
print('max yloc:', max(yloc_list))
...
for layer in list(congestion_data['layerList']):
    ...
    for idx in range(num_instances):
        if demand_list[idx] > capacity_list[idx]:
            red_x.append(xloc_list[idx])
            red_y.append(yloc_list[idx])
            red_color.append('red')
        else:
            blue_x.append(xloc_list[idx])
            blue_y.append(yloc_list[idx])
            blue_color.append('blue')
    
    print('Start visualization')
    plt.scatter(blue_x, blue_y, c = blue_color)
    plt.scatter(red_x, red_y, c = red_color)
    plt.title('Congestion on layer ' + layer + ' of design ' + design)
    plt.savefig(layer + '_' + design + '.png')
