    ...
    def forward(self, batched_data):
        ...
        for layer in range(self.num_layer):
            if self.gnn_type == 'hyper':
                h = self.convs[layer](h_list[layer], net_inst_adj, inst_net_adj_v_drive, inst_net_adj_v_sink)
...
class GNN_node_Virtualnode(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "concat", residual = True, gnn_type = 'gin', norm_type = "layer",
        ...
        ### List of Convs to aggregate message 
        self.vn_convs = torch.nn.ModuleList()
        ...
        for layer in range(num_layer - 1):
            self.vn_convs.append(GATv2Conv(in_channels=emb_dim, out_channels=emb_dim, heads=3, concat=False))
            #self.top_convs.append(GATv2Conv(in_channels=emb_dim, out_channels=emb_dim, heads=2, concat=False, aggr="mean"))
    ...
    def forward(self, batched_data):
        ...
        x, edge_index, edge_attr, edge_index_local_vn, edge_index_vn_top, num_vn, num_top_vn, batch = batched_data.x, batched_data.edge_index, batched_data.edge_attr, batched_data.edge_index_local_vn, batched_data.edge_index_vn_top, batched_data.num_vn, batched_data.num_top_vn, batched_data.batch
        
        virtualnode_embedding = self.virtualnode_embedding(torch.zeros(num_vn).to(edge_index.dtype).to(edge_index.device))
        #top_embedding = self.virtualnode_embedding(torch.zeros(num_top_vn).to(edge_index.dtype).to(edge_index.device))
        ...
        for layer in range(self.num_layer):
            ...
            #virtualnode_embedding = virtualnode_embedding + top_embedding[edge_index_vn_top[1]]
            h_list[layer] = h_list[layer] + self.vn_convs[layer - 1]((virtualnode_embedding, h_list[layer]), edge_index_local_vn.flip([0]))
            ...
            if layer < self.num_layer - 1:
                ...
                #virtualnode_embedding_temp = self.vn_convs[layer]((h, virtualnode_embedding), edge_index_local_vn) 
                ...
                ### add message from virtual nodes to top node
                #top_embedding_temp = self.top_convs[layer]((virtualnode_embedding, top_embedding), edge_index_vn_top) 
                #if self.residual:
