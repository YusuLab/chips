...
# Multiresolution Graph Networks
class MGN(nn.Module):
    def __init__(self, clusters, num_layers, node_dim, edge_dim, hidden_dim, z_dim, num_outputs = 1, regression = True, position_encoder = None, device = 'cuda'):
        ...
        if self.position_encoder is not None:
            self.node_dim += self.position_encoder.num_vectors
    ...
    def forward(self, adj, node_feat, edge_feat = None):
        # Position encoding
        if self.position_encoder is not None:
            position = self.position_encoder(adj)
            node_feat = torch.cat((node_feat, position), dim = 2)
...
class GraphEncoder(nn.Module):
    def __init__(self, num_layers, node_dim, edge_dim, hidden_dim, z_dim, position_encoder = None, use_concat_layer = True, device = 'cuda', **kwargs):
