...
class pyg_dataset(Dataset):
    def __init__(self, data_dir, fold_index, split, target, load_pe = False, num_eigen = 10, load_global_info = False, load_pd = False, graph_rep = 'bipartite', vn=False, net = True, concat = False, design = 19, pl = 0):
        super().__init__()
...
#             # Load capacity
            ...
            if vn:       
                ...
                if pl:
                    file_name = data_dir + '/' + str(graph_index) + '.pl_fix_part_dict.pkl'
                f = open(file_name, 'rb')
            ...
            if self.load_global_info == True:
                ...
                global_info = torch.Tensor([num_nodes])
