...
if pe_type == 'lap':
    ...
    print('Number of input features + eigenvectors:', input_dim)
...
model = Linear_Transformer(input_dim = input_dim, hidden_dim = args.hidden_dim, output_dim = num_outputs, max_seq_len = max_seq_len, heads = args.heads, depth = args.depth, n_local_attn_heads = args.local_heads).to(device = device)
...
for epoch in range(num_epoch):
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        # Position encoding
        if pe_type == 'lap':
            feat = torch.cat([feat, data.evects], dim = 1)
