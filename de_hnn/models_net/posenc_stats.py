...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    if 'LapPE' in pe_types or 'EquivStableLapPE' in pe_types:
        ...
        # Precomputed
        if hasattr(data, 'evals') and hasattr(data, 'evects'):
            evals = np.zeros(N)
            evects = np.zeros((N, N))
            batch_size = data.ptr.size(0) - 1

            precomputed_evals = data.evals.cpu().numpy()
            precomputed_evects = data.evects.cpu().numpy()
            
            stride = precomputed_evals.shape[0] // batch_size
            for b in range(batch_size):
                start = data.ptr[b]
                finish = data.ptr[b + 1]
                n = finish - start
                evals[start : finish] = precomputed_evals[b * stride : b * stride + n]
                evects[start : finish, start : finish] = precomputed_evects[start : finish, : n]
            
            dtype = L.toarray().dtype
            evals = evals.astype(dtype)
            evects = evects.astype(dtype)
        else:
            # Numpy
            evals, evects = np.linalg.eigh(L.toarray())

            # Sparse Scipy
            # evals, evects = eigsh(L, k = cfg.posenc_LapPE.dim_pe)
    ...
    if 'SignNet' in pe_types:
        ...
        # Sparse Scipy
        evals_sn, evects_sn = eigsh(L, k = cfg.posenc_SignNet.dim_pe)
