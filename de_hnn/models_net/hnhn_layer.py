...
class HNHNLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if incidence_1 is not None:
            ...
            self.incidence_1_transpose = None
            self.use_normalized_incidence = False
    ...
    def forward(self, x_0, incidence_1_transpose=None, incidence_1=None):
        ...
        if self.incidence_1 is None:
            incidence_1 = incidence_1.to(x_0.device)
            incidence_1_transpose = incidence_1.transpose(1, 0).to(x_0.device)
        else:
            incidence_1 = self.incidence_1.to(x_0.device)
            incidence_1_transpose = self.incidence_1_transpose.to(x_0.device)
        
        # Compute output hyperedge features
        x_1 = self.conv_0_to_1(x_0, incidence_1)  # nodes to edges
        if self.use_bias:
            x_1 += self.bias_0_to_1
        # Compute output hypernode features
        x_0 = self.conv_1_to_0(x_1, incidence_1_transpose)  # edges to nodes
        if self.use_bias:
