...
class GNN_node(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "concat", residual = True, gnn_type = 'gin', norm_type = "layer",
        ...
        if self.JK == "concat":
            self.fc1 = torch.nn.Linear((self.num_layer + 1) * emb_dim * 2, (self.num_layer + 1) * emb_dim)
                                
        else:
            self.fc1 = torch.nn.Linear(emb_dim * 2, emd_dim)
        ...
        self.re_norms = torch.nn.ModuleList()
        ...
        for layer in range(num_layer):
            ...
            #self.batch_norms.append(torch.nn.BatchNorm1d(emb_dim))
            if norm_type == "batch":
                self.re_norms.append(torch.nn.BatchNorm1d(emb_dim))
            elif norm_type == "layer":
                self.re_norms.append(torch.nn.LayerNorm(emb_dim))
            else:
                raise NotImplemented
    ...
    def forward(self, batched_data):
        ...
        for layer in range(self.num_layer):
            ...
            h_re = self.re_convs[layer](h_list_re[layer], edge_index_rev, edge_attr)
            ...
            h_list.append(h)
            h_list_re.append(h_re)

        ### Different implementations of Jk-concat
        if self.JK == "last":
            node_representation = h_list[-1]
            node_representation_re = h_list_re[-1]
        elif self.JK == "sum":
            node_representation = 0
            node_representation_re = 0
            for layer in range(self.num_layer + 1):
                node_representation += h_list[layer]
                node_representation_re += h_list_re[layer]
        elif self.JK == "concat":
            node_representation = torch.cat(h_list, dim = 1)
            node_representation_re = torch.cat(h_list_re, dim = 1)

        output = torch.cat([node_representation, node_representation_re], dim=1)
        output = self.fc1(h_node)
        return output
